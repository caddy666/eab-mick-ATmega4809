...
/**
 * @brief  Return 1 if the focus servo is locked.
 *
 * Reads the FOK GPIO via fok_locked() — faster than a serial DSIC2 poll and
 * has the correct polarity (HIGH = locked).  The previous rd_dsic2() path
 * had an inverted return: `(FE & 1) ? 1 : 0` returned 1 when FE=1 (out of
 * focus) — the opposite of what every call site expected.
 */
static uint8_t dsic_in_focus(void)
{
    return (uint8_t)fok_locked();
}
