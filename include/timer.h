 ...
 *
 * Renamed from delay() to avoid conflicting with Arduino's delay(unsigned long).
 * Uses delayMicroseconds() internally; the CPU is blocked for the entire
 * duration.  Only used during the power-on reset sequence in driver.cpp.
 */
